...
#include "cli.h"
#include "cli_gui.h"
#include "eeprom.h" // TODO: 06/07 @PARK 
...
#define AK4183_TOUCH_HEIGTH   HW_LCD_HEIGHT
...
static bool ak4183touchDataWrite(ak4183_cali_t* p_data);
static bool ak4183touchDataRead(ak4183_cali_t* p_data);
...
// * 수정하면 안되는 절대적인 기준 (좌표값)
static uint32_t x_scr_ref[5] = {90, 90, 700, 700, 400};
static uint32_t y_scr_ref[5] = {90, 400, 400, 90, 250};

/*
static uint16_t x_buf[5] = {512,  529,  3535, 3535, 2087};
static uint16_t y_buf[5] = {1011, 3376, 3367, 993,  2243};
*/
// static ak4183_adc_t adc_buf[5];
...
{
  bool ret = false;
  ak4183_adc_t adc_tmp;

  if (ak4183ReadAdc(&adc_tmp))
  {
    *x_adc = adc_tmp.x_adc;
    *y_adc = adc_tmp.y_adc;
    ret = true;
...
{
  bool ret = false;
  ak4183_cali_t cali_data;

  memcpy(cali_data.x_adc, tch_info.x_adc, sizeof(uint32_t) * 5);
  memcpy(cali_data.y_adc, tch_info.y_adc, sizeof(uint32_t) * 5);
  cali_data.tch_magic_number = AK4183_EEPROM_MAGIC_NUMBER;

  if (eepromIsInit())
  {
    ak4183touchDataWrite(&cali_data);
    ret = true;
  }
...
bool ak4183Init(void)
{
  ...
  if (eepromIsInit())
  {
    ...
    {
      logPrintf("[ak4183] magic number failed\n");
...
void cliCmd(cli_args_t *args)
{
  ...
  {
    cliPrintf("Current AK4183 Touch ADC Data\n");
    cliPrintf("==================================\n");
    for (uint8_t i=0; i<5; i++)
    {
      cliPrintf("ADC X  [%4d] Y   [%4d]\n", tch_adc.x_adc[i], tch_adc.y_adc[i]);
    }
    cliPrintf("==================================\n");
  ...
  if ((args->argc == 12) && args->isStr(0, "set") && args->isStr(1, "adc")
            && args->getData(2)   // x0
            && args->getData(3)   // x1
            && args->getData(4)   // x2
            && args->getData(5)   // x3
  ...
  {
    ak4183_cali_t adc_tmp;
    
    for (uint8_t i=0; i<5; i++)
    {
      adc_tmp.x_adc[i] = args->getData(i+2);
      adc_tmp.y_adc[i] = args->getData(i+7);
    }

    for (uint8_t i=0; i<5; i++)
    {
      cliPrintf("adc_tmp -> x_adc[%d] : %4d,    y_adc[%d] : %4d\n", i, adc_tmp.x_adc[i], i, adc_tmp.y_adc[i]);
    }

    ak4183touchDataWrite(&adc_tmp);
    ak4183touchDataRead(&tch_adc);
    ret = true;
  }
  ...
  if (ret == false)
  {
    ...
    cliPrintf("ak4183 set adc [x0] [x1] [x2] [x3] [x4] [y0] [y1] [y2] [y3] [y4]\n");
  }
