...
const can_baud_cfg_t can_baud_cfg_80m_normal[] = { { 50, 8, 13, 2 }, // 100K, 87.5%
		{ 40, 8, 13, 2 }, // 125K, 87.5%
		{ 20, 8, 13, 2 }, // 250K, 87.5%
		{ 10, 8, 13, 2 }, // 500K, 87.5%
		{ 5, 8, 13, 2 }, // 1M,   87.5%
...
const can_baud_cfg_t can_baud_cfg_80m_data[] = { { 40, 8, 11, 8 }, // 100K, 60%
		{ 32, 8, 11, 8 }, // 125K, 60%
		{ 16, 8, 11, 8 }, // 250K, 60%
		{ 8, 8, 11, 8 }, // 500K, 60%
		{ 4, 8, 11, 8 }, // 1M,   60%
...
const can_baud_cfg_t can_baud_cfg_150m_normal[] = { { 100, 8, 12, 2 }, // 100K, 86.7%
		{ 75, 8, 13, 2 }, // 125K, 87.5%
		{ 40, 8, 12, 2 }, // 250K, 87.5%
		{ 20, 8, 12, 2 }, // 500K, 86.7%
		{ 10, 8, 8, 2 }, // 1M,   86.7%
...
const can_baud_cfg_t can_baud_cfg_150m_data[] = { { 75, 8, 14, 5 }, // 100K, 75%
		{ 60, 8, 14, 5 }, // 125K, 75%
		{ 30, 8, 14, 5 }, // 250K, 75%
		{ 15, 8, 14, 5 }, // 500K, 75%
		{ 10, 8, 10, 4 }, // 1M,   75%
		{ 5, 8, 10, 4 }, // 2M    75%
		{ 1, 8, 11, 8 },  // 4M    60% not support
		{ 2, 8, 10, 4 }, // 5M    73.3%
		};
...
const uint32_t dlc_len_tbl[] = { 0, 1, 2, 3, 4, 5, 6, 7, 8, 12, 16, 20, 24, 32,
		48, 64 };
...
bool canMsgInit(can_msg_t *p_msg, CanFrame_t frame, CanIdType_t id_type,
		CanDlc_t dlc) {
	p_msg->frame = frame;
...
void canAttachRxInterrupt(uint8_t ch,
bool (*handler)(uint8_t ch, CanEvent_t evt, can_msg_t *arg)) {
	if (ch > CAN_MAX_CH)
		return;
